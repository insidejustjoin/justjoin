...
// 管理者用：全ユーザーに通知を送信
export async function sendNotificationToAllUsers(title: string, message: string, type: 'info' | 'success' | 'warning' | 'error' = 'info'): Promise<void> {
  const client = await pool.connect();
  try {
    // すべてのユーザーIDを取得
...
export async function sendProfileCompletionNotification(userId: string, userName: string): Promise<void> {
  await createNotification({
    user_id: userId,
...
// 自動通知：書類作成100%の通知
export async function sendDocumentCompletionNotification(userId: string, userName: string): Promise<void> {
  await createNotification({
    user_id: userId,
...
// スポット通知履歴を取得
export async function getSpotNotificationHistory(): Promise<any[]> {
  const client = await pool.connect();
  try {
    const result = await client.query(`
...
// ワークフロー通知履歴を取得
export async function getWorkflowNotificationHistory(): Promise<any[]> {
  const client = await pool.connect();
  try {
    const result = await client.query(`
...
// スポット通知履歴を削除
export async function deleteSpotNotificationHistory(notificationId: string): Promise<void> {
  const client = await pool.connect();
  try {
    await client.query(`
...
// スポット通知履歴を更新
export async function updateSpotNotificationHistory(notificationId: string, data: {
  title: string;
  message: string;
  type: 'info' | 'success' | 'warning' | 'error';
}): Promise<void> {
...
// 履歴IDに基づいて求職者の通知を更新
export async function updateUserNotificationsByHistoryId(historyId: string, data: {
  title: string;
  message: string;
  type: 'info' | 'success' | 'warning' | 'error';
}): Promise<void> {
...
// 履歴IDに基づいて求職者の通知を削除
export async function deleteUserNotificationsByHistoryId(historyId: string): Promise<void> {
  const client = await pool.connect();
  try {
    // 履歴から通知IDを取得して通知を削除
    const historyResult = await client.query(`
