...
export interface CreateJobSeekerData {
  user_id: string;
  full_name?: string;
  date_of_birth?: string;
  gender?: 'male' | 'female' | 'other';
...
export interface UpdateJobSeekerData {
  full_name?: string;
  date_of_birth?: string;
  gender?: 'male' | 'female' | 'other';
  email?: string;
...
export const jobSeekersRepository = {
  ...
  // 求職者プロフィールを更新
  async update(userId: string, data: UpdateJobSeekerData): Promise<JobSeeker | null> {
    if (!isServer) {
  ...
  // 全求職者を取得（管理者用）
  async getAll(): Promise<JobSeeker[]> {
    if (!isServer) {
