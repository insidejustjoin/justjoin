...
router.post('/verify-email', async (req, res) => {
  try {
    ...
    try {
      await query(
        `INSERT INTO email_verifications (email, first_name, last_name, verification_token, expires_at, created_at)
      ...
      if (dbError.message?.includes('does not exist')) {
        ...
        `);
        await query(
          `INSERT INTO email_verifications (email, first_name, last_name, verification_token, expires_at)
           VALUES ($1, $2, $3, $4, $5)`,
          [email, firstName, lastName, verificationToken, expiresAt]
        );
      } else {
...
router.get('/verify/:token', async (req, res) => {
  try {
    ...
    const result = await query(
      `SELECT email, first_name, last_name, verified, expires_at
