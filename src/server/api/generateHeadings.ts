...
interface GenerateHeadingsRequest {
  content: string;
}

interface GenerateHeadingsResponse {
  success: boolean;
  headings?: string[];
  error?: string;
}

export async function generateHeadings(req: Request, res: Response) {
  try {
    const { content }: GenerateHeadingsRequest = req.body;
