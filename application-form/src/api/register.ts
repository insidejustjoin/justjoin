...
// 求職者登録API
export async function registerJobSeekerAPI(email: string, firstName: string, lastName: string, language: 'ja' | 'en' = 'ja') {
  // ブラウザ環境では実行しない
  ...
  try {
    ...
    // 管理者への新規登録通知メール送信
    try {
      const adminEmailSent = await emailService.sendAdminNewRegistrationNotification(
