...
// 多言語対応Excelファイル生成API
export const generateMultilingualExcel = async (
  request: ExcelGenerationRequest
): Promise<ExcelGenerationResponse> => {
  try {
    ...
    const typedResult = result as ExcelGenerationResponse;
...
// ExcelファイルダウンロードAPI
export const downloadExcelFile = async (filePath: string): Promise<Blob> => {
  try {
...
// 多言語表示用のヘルパー関数
export const getMultilingualText = (key: string, language: 'ja' | 'en' = 'ja'): string => {
  const translations = {
    ja: {
