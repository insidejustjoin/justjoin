...
import { hashPassword, verifyPassword, generateRandomPassword, generateJWT, verifyJWT } from '../../utils/auth.js';
...
export const authRepository = {
  // ユーザー登録（求職者）
  async registerJobSeeker(data: RegisterJobSeekerData): Promise<{ user: AuthUser; password: string }> {
    if (!isServer) {
  ...
  // ユーザー登録（企業）
  async registerCompany(data: RegisterCompanyData): Promise<AuthUser> {
    if (!isServer) {
      // フロントエンドでは実際のAPIを呼び出す
      try {
        const apiUrl = process.env.NODE_ENV === 'development' ? 'http://localhost:3001' : 'https://justjoin.jp';
    ...
    try {
      ...
      const adminNotificationSent = await emailService.sendAdminCompanyRegistrationNotification(
        adminEmail, 
        data.companyName, 
        data.email, 
        data.description
  ...
  // 企業承認
  async approveCompany(userId: string): Promise<{ user: AuthUser; password: string }> {
    if (!isServer) {
  ...
  // 企業却下
  async rejectCompany(userId: string, reason: string = '審査基準を満たしていませんでした'): Promise<AuthUser> {
    if (!isServer) {
      const mockUser = mockUsers.find(u => u.id === userId);
      if (!mockUser) throw new Error('User not found');
  ...
  // 承認待ち企業一覧取得
  async getPendingCompanies(): Promise<AuthUser[]> {
    if (!isServer) {
  ...
  // パスワード再発行（求職者）
  async resetJobSeekerPassword(email: string, language: 'ja' | 'en' = 'ja'): Promise<{ success: boolean; message: string }> {
    if (!isServer) {
  ...
  // パスワード再発行（企業）
  async resetCompanyPassword(email: string, language: 'ja' | 'en' = 'ja'): Promise<{ success: boolean; message: string }> {
    if (!isServer) {
    ...
    try {
      ...
      const userResult = await query(
        'SELECT id, email FROM users WHERE email = $1 AND user_type = $2',
        [email, 'company']
      );
      ...
      // パスワード変更
    async changePassword(userId: string | number, currentPassword: string, newPassword: string, language: 'ja' | 'en' = 'ja'): Promise<{ success: boolean; message: string }> {
      if (!isServer) {
        const mockUser = mockUsers.find(u => u.id === userId);
        if (!mockUser) {
    ...
    try {
      ...
      try {
        const jobSeekerResult = await query(
          'SELECT first_name, last_name FROM job_seekers WHERE user_id = $1',
          [userId]
        );
  ...
  // プロフィール更新
  async updateProfile(userId: string, profileData: any): Promise<{ success: boolean; message: string; profile?: any }> {
    if (!isServer) {
      // ブラウザ環境ではAPIを呼び出す
      try {
  ...
  // プロフィール取得
  async getProfile(userId: string): Promise<{ success: boolean; profile?: any; message?: string }> {
    if (!isServer) {
      // ブラウザ環境ではAPIを呼び出す
      try {
        const token = localStorage.getItem('auth_token');
  ...
  // 管理者作成（制限付き）
  async createAdmin(email: string, password?: string): Promise<{ user: AuthUser; password: string }> {
  ...
  // 管理者パスワードリセット
  async resetAdminPassword(adminId: string): Promise<{ user: AuthUser; password: string }> {
    if (!isServer) {
      const mockUser = mockUsers.find(u => u.id === adminId && u.user_type === 'admin');
      if (!mockUser) throw new Error('Admin not found');
