...
app.get('/api/jobseekers/profile-from-documents/:userId', async (req, res) => {
    try {
        ...
        const documentResult = await query('SELECT document_data FROM user_documents WHERE user_id = $1 ORDER BY created_at DESC LIMIT 1', [userId]);
...
app.get('/api/jobseekers/completion-rate/:userId', async (req, res) => {
    try {
        ...
        const normalizeRegistrationType = (value) => {
        ...
        const jsResult = await query(`SELECT completion_rate, registration_type 
         FROM job_seekers 
    ...
    `;
        ...
        if (docResult.rows.length > 0 && docResult.rows[0].document_data && calculateCompletionRate) {
            ...
            const docRegistrationType = normalizeRegistrationType(docResult.rows[0].registration_type || registrationTypeFilter || existingRegistrationType);
...
app.get('/api/jobseekers/registration-types/:userId', async (req, res) => {
    try {
        ...
        // job_seekers
        try {
            const js = await query(`SELECT COALESCE(registration_type, 'engineer') AS registration_type
           FROM job_seekers
          WHERE user_id = $1`, [userId]);
            js.rows.forEach((r) => all.push(r.registration_type));
        }
        catch (e) {
            console.warn('[registration-types] job_seekers query failed:', e?.message || e);
        }
        // user_documents
        try {
            const docs = await query(`SELECT DISTINCT COALESCE(registration_type, 'engineer') AS registration_type
           FROM user_documents
          WHERE user_id = $1`, [userId]);
            docs.rows.forEach((r) => all.push(r.registration_type));
        }
        catch (e) {
            console.warn('[registration-types] user_documents query failed:', e?.message || e);
        }
        ...
        try {
            ...
            if (userResult.rows.length > 0) {
                ...
                const emailVerResult = await query(`SELECT verified FROM email_verifications WHERE email = $1 AND verified = true`, [email]);
                if (emailVerResult.rows.length > 0) {
                    // メール確認済みの場合は、job_seekersテーブルから登録タイプを取得
                    const jsResult = await query(`SELECT DISTINCT COALESCE(registration_type, 'engineer') AS registration_type
             FROM job_seekers WHERE user_id = $1`, [userId]);
                    jsResult.rows.forEach((r) => all.push(r.registration_type));
                }
            }
        }
        catch (e) {
            console.warn('[registration-types] email_verifications query failed:', e?.message || e);
        }
        const types = Array.from(new Set(all
            .filter(Boolean)
            .map((t) => (String(t).toLowerCase().trim() === 'general' ? 'general' : 'engineer'))));
...
app.post('/api/admin/login', async (req, res) => {
    ...
    `, [email]);
        ...
        const token = jwt.default.sign({
            ...
            email: user.email,
            role: user.role
        }, process.env.JWT_SECRET || 'justjoin-jwt-secret-2024', { expiresIn: '24h' });
        res.json({
            success: true,
            token,
            user: {
...
app.get('/api/admin/jobseekers', async (req, res) => {
    try {
            ...
            return Promise.all(result.rows.map(async (row) => {
                ...
                if (row.document_data && calculateCompletionRate) {
                 ...
                 SET completion_rate = $1, 
                     ...
                     END,
                     updated_at = NOW() 
                 WHERE user_id = $2 AND LOWER(COALESCE(registration_type, 'engineer')) = LOWER($3)`, [calculated, row.user_id, regType, shouldEnableInterview]).catch(err => console.warn('Completion rate update failed:', err));
                        }
                    }
                    catch (err) {
        ...
        const buildMinimalQuery = async (type) => {
            let whereClause = 'WHERE 1=1';
            const params = [];
            if (type === 'engineer') {
                whereClause += ` AND (js.registration_type IS NULL OR LOWER(js.registration_type) = $${params.length + 1})`;
          ...
          SELECT
            ...
            latest.reason,
            latest.notes,
            -- 最新の音声録音があるかどうか（buildMinimalQueryでは常にfalse）
        ...
        res.status(500).json({ success: false, message: '求職者一覧の取得に失敗しました', detail: error?.message || null });
    }
});
// 管理者用：求職者詳細取得API
app.get('/api/jobseekers/:id', async (req, res) => {
    try {
        ...
        const normalizeType = (v) => {
            if (typeof v === 'string' && v.trim().toLowerCase() === 'general')
                return 'general';
            return 'engineer';
        };
...
app.get('/api/jobseekers/profile/:userId', async (req, res) => {
    try {
        const { userId } = req.params;
        const { registrationType } = req.query;
        const normalizeType = (v) => typeof v === 'string' && v.trim().toLowerCase() === 'general' ? 'general' : 'engineer';
        const type = normalizeType(registrationType);
        const { query } = await import('../integrations/postgres/client.js');
        ...
        if (row) {
          ...
          `, [userId, type]);
                if (doc.rows.length > 0) {
                    ...
                    const documentData = typeof dd === 'string'
                        ? (() => {
                            try {
...
app.delete('/api/admin/jobseekers/:id', authenticate, async (req, res) => {
    try {
        ...
        const registrationType = req.query.registration_type; // 'engineer' | 'general' | undefined
        const { query } = await import('../integrations/postgres/client.js');
        ...
        try {
            ...
            const remainingCountResult = await query('SELECT COUNT(*) as count FROM job_seekers WHERE user_id = $1', [userId]);
      ...
      `, [userId]);
        ...
        try {
            ...
            if (user.user_type === 'job_seeker') {
                ...
                const jobSeekerResult = await query('SELECT id, registration_type FROM job_seekers WHERE user_id = $1 AND LOWER(COALESCE(registration_type, \'engineer\')) = LOWER($2)', [userId, normalizedRegistrationType]);
          ...
          `, [userId, normalizedRegistrationType]);
                ...
                await query('DELETE FROM user_documents WHERE user_id = $1 AND LOWER(COALESCE(registration_type, \'engineer\')) = LOWER($2)', [userId, normalizedRegistrationType]);
                ...
                await query('DELETE FROM job_seekers WHERE user_id = $1 AND LOWER(COALESCE(registration_type, \'engineer\')) = LOWER($2)', [userId, normalizedRegistrationType]);
...
// 統一ログインAPI（求職者・企業・管理者対応）- メールアドレスベース
    ...
    try {
      ...
      `, [email]);
        ...
        if (user.role === 'job_seeker') {
            ...
            WHERE user_id = $1
          `, [user.id]);
                registrationTypes = Array.from(new Set(typesResult.rows.map((row) => (row.registration_type === 'general' ? 'general' : 'engineer'))));
            }
            catch (typeError) {
            ...
            SELECT DISTINCT COALESCE(registration_type, 'engineer') AS registration_type
             ...
             WHERE user_id = $1
          `, [user.id]);
                const docTypes = docTypeResult.rows.map((row) => row.registration_type === 'general' ? 'general' : 'engineer');
            ...
            if (registrationType) {
                const normalized = registrationType === 'general' || registrationType === 'engineer' ? registrationType : 'engineer';
        ...
        }, process.env.JWT_SECRET || 'justjoin-jwt-secret-2024', { expiresIn: '8h' } // 8時間に短縮
        );
...
app.post('/api/register-company', async (req, res) => {
    ...
    `, [user.id, companyName, description || '']);
        ...
        try {
            const { emailService } = await import('../services/emailService.js');
            await emailService.sendAdminNewRegistrationNotification('inside.justjoin@gmail.com', 'company', {
                email: email,
...
const ensureJobSeekerColumns = async (queryFn) => {
    if (jobSeekerColumnsEnsured)
        return;
    try {
        await queryFn(`
