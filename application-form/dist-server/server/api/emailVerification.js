...
router.post('/verify-email', async (req, res) => {
    try {
        ...
        try {
            ...
            // まず既存レコードがあるかチェック
            const existingResult = await query(`SELECT id, email FROM email_verifications WHERE email = $1`, [email]);
            ...
            if (existingResult.rows.length > 0) {
             ...
             SET first_name = $1,
                 ...
                 verified = false,
                 updated_at = NOW()
             WHERE email = $6`, [firstName, lastName, verificationCode, tokenValue, expiresAt, email]);
                }
                else {
                    await query(`UPDATE email_verifications 
             SET first_name = $1,
             ...
             WHERE email = $5`, [firstName, lastName, verificationCode, expiresAt, email]);
                ...
                if (hasVerificationTokenColumn) {
                    await query(`INSERT INTO email_verifications (email, first_name, last_name, verification_code, verification_token, expires_at, created_at)
             VALUES ($1, $2, $3, $4, $5, $6, NOW())`, [email, firstName, lastName, verificationCode, tokenValue, expiresAt]);
                }
                else {
...
router.get('/check/:email', async (req, res) => {
    try {
        ...
        const result = await query(`SELECT verified, expires_at, updated_at
       FROM email_verifications
       WHERE email = $1
       ORDER BY created_at DESC
       LIMIT 1`, [email]);
