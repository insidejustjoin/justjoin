...
router.get('/google/callback', async (req, res) => {
    ...
    `, [email]);
        ...
        `, [email, 'job_seeker', 'active']);
                ...
                const existingJobSeeker = await client.query('SELECT id FROM job_seekers WHERE user_id = $1 LIMIT 1', [userId]);
        ...
        if (userType === 'job_seeker') {
          ...
          `, [userId]);
                if (jobSeekerResult.rows.length > 0) {
                    ...
                    registrationTypes = Array.from(new Set(jobSeekerResult.rows.map((r) => (r.registration_type === 'general' ? 'general' : 'engineer'))));
    ...
    `, [userId]);
        ...
        }, process.env.JWT_SECRET || 'justjoin-jwt-secret-2024', { expiresIn: '8h' });
