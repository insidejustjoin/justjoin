...
router.post('/check', rateLimit(10, 60000), async (req, res) => {
    try {
        ...
        try {
         ...
         WHERE js.phone = $1`, [normalizedPhone]);
        }
        catch (dbErr) {
           ...
           VALUES ($1, 'active', '再登録によりアクティブ化')`, [userId]);
            ...
            try {
                if (documentsData) {
                    ...
                    const existingDoc = await query('SELECT id FROM user_documents WHERE user_id = $1 AND document_type = $2 AND registration_type = $3', [userId.toString(), 'resume', 'engineer']);
                    if (existingDoc.rows.length > 0) {
                        await query(`UPDATE user_documents 
               SET document_data = $1, updated_at = NOW() 
               WHERE user_id = $2 AND document_type = $3 AND registration_type = $4`, [JSON.stringify(finalDocumentsData), userId.toString(), 'resume', 'engineer']);
                    }
                    else {
                        await query(`INSERT INTO user_documents (user_id, document_type, registration_type, document_data, created_at, updated_at)
               VALUES ($1, $2, $3, $4, NOW(), NOW())`, [userId.toString(), 'resume', 'engineer', JSON.stringify(finalDocumentsData)]);
                    }
                }
            }
            catch (docErr) {
            ...
            if (documentsData) {
                ...
                const filledFields = requiredFields.filter(field => documentsData[field] || (documentsData.resume && documentsData.resume[field])).length;
                completionRate = Math.round((filledFields / requiredFields.length) * 100);
            }
            // completion_rateを更新
            try {
                await query('UPDATE job_seekers SET completion_rate = $1 WHERE user_id = $2', [completionRate, userId]);
            }
            catch {
           ...
           VALUES ($1, 'active', '再登録によりアクティブ化')`, [userId]);
            ...
            try {
                if (documentsData) {
                    ...
                    const existingDoc = await query('SELECT id FROM user_documents WHERE user_id = $1 AND document_type = $2 AND registration_type = $3', [userId.toString(), 'resume', 'general']);
                    if (existingDoc.rows.length > 0) {
                        await query(`UPDATE user_documents 
               SET document_data = $1, updated_at = NOW() 
               WHERE user_id = $2 AND document_type = $3 AND registration_type = $4`, [JSON.stringify(finalDocumentsData), userId.toString(), 'resume', 'general']);
                    }
                    else {
                        await query(`INSERT INTO user_documents (user_id, document_type, registration_type, document_data, created_at, updated_at)
               VALUES ($1, $2, $3, $4, NOW(), NOW())`, [userId.toString(), 'resume', 'general', JSON.stringify(finalDocumentsData)]);
                    }
                }
            }
            catch (docErr) {
