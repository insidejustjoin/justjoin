...
app.get('/api/jobseekers/registration-types/:userId', async (req, res) => {
    try {
        ...
        try {
            const temps = await query(`SELECT DISTINCT COALESCE(tr.registration_type, 'engineer') AS registration_type
           FROM temporary_registrations tr
           JOIN users u ON u.email = tr.email
          WHERE u.id = $1
            AND tr.status IN ('documents_completed','completed')`, [userId]);
            temps.rows.forEach((r) => all.push(r.registration_type));
        }
        catch (e) {
            console.warn('[registration-types] temporary_registrations query failed:', e?.message || e);
        }
