        ...
        `, params);
            ...
            return Promise.all(result.rows.map(async (row) => {
                ...
                if (row.document_data && calculateCompletionRate) {
                 ...
                 WHERE user_id = $2 AND LOWER(COALESCE(registration_type, 'engineer')) = LOWER($3)`, [calculated, row.user_id, regType]).catch(err => console.warn('Completion rate update failed:', err));
                        }
                    }
                    catch (err) {
