...
router.post('/temporary', async (req, res) => {
    try {
        ...
        // 既存の仮登録チェック
        const existingTemp = await query('SELECT id FROM temporary_registrations WHERE email = $1 AND status != $2', [email, 'completed']);
...
router.post('/complete/:token', async (req, res) => {
    try {
       ...
       WHERE verification_token = $1 AND expires_at > NOW() AND status = $2`, [token, 'documents_completed']);
        ...
        // ユーザー作成
        const userResult = await query(`INSERT INTO users (email, password_hash, role, created_at, updated_at) 
       VALUES ($1, $2, $3, NOW(), NOW()) RETURNING id`, [registration.email, passwordHash, 'jobseeker']);
