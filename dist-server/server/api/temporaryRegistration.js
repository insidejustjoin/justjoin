...
router.post('/temporary', async (req, res) => {
    try {
        ...
        const existingUser = await query('SELECT id, status FROM users WHERE email = $1', [email]);
...
router.get('/verify/:token', async (req, res) => {
    try {
       ...
       WHERE verification_token = $1 AND status IN ($2, $3) AND expires_at > NOW()`, [token, 'pending', 'documents_completed']);
...
router.post('/documents/:token', async (req, res) => {
    try {
        ...
        // 仮登録データ取得
        const tempReg = await query(`SELECT * FROM temporary_registrations 
       WHERE verification_token = $1 AND status = $2 AND expires_at > NOW() AND expires_at > NOW()`, [token, 'pending']);
        if (tempReg.rows.length === 0) {
            return res.status(400).json({
                success: false,
                message: '無効なトークンまたは期限切れです。'
...
router.post('/complete/:token', async (req, res) => {
    try {
      ...
      `);
       ...
       WHERE verification_token = $2`, ['completed', token]);
        res.json({
            success: true,
            message: '登録が完了しました。',
            data: {
