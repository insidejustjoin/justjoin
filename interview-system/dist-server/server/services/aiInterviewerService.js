...
exports.AIInterviewerService = void 0;
...
class AIInterviewerService {
     ...
     * 質問に対する自然な導入メッセージを生成
     */
    generateQuestionIntroduction(question, language, isFirst = false) {
        if (isFirst) {
            const intros = {
     ...
     * 回答に対するリアクションを生成
     */
    generateResponseReaction(answer, language) {
     ...
     * 面接完了時のメッセージを生成
     */
    generateCompletionMessage(session, language) {
     ...
     * 次の質問を取得し、AIレスポンスを生成
     */
    async getNextQuestionResponse(session, lastAnswer) {
     ...
     * 面接サマリーを生成
     */
    generateInterviewSummary(session) {
    ...
    extractKeyInsights(answers, language) {
        // 簡易的なキーワード抽出
        const insights = [];
        answers.forEach(answer => {
            if (answer.text.length > 100) {
                if (language === 'ja') {
                    if (answer.text.includes('チーム'))
                        insights.push('チームワークを重視');
                    if (answer.text.includes('学習') || answer.text.includes('勉強'))
                        insights.push('学習意欲が高い');
    ...
    extractStrengths(answers, language) {
        const strengths = [];
        answers.forEach(answer => {
    ...
    extractAreasForImprovement(answers, language) {
        const improvements = [];
...
exports.AIInterviewerService = AIInterviewerService;
//# sourceMappingURL=aiInterviewerService.js.map