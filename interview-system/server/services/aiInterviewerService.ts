...
export class AIInterviewerService {
  ...
  generateQuestionIntroduction(question: Question, language: Language, isFirst: boolean = false): string {
    ...
    const randomIntros: Record<Language, string[]> = {
      ja: [
  ...
  generateResponseReaction(answer: Answer, language: Language): string {
    ...
    const reactions: Record<Language, { positive: string[]; encouraging: string[]; neutral: string[] }> = {
      ja: {
        positive: [
    ...
    answers.forEach(answer => {
      if (answer.text.length > 100) {
        if (language === 'ja') {
          ...
          if (answer.text.includes('команда') || answer.text.includes('команд')) insights.push('Ценит командную работу');
          if (answer.text.includes('обучение') || answer.text.includes('изучение')) insights.push('Высокая мотивация к обучению');
          if (answer.text.includes('рост')) insights.push('Ориентация на рост');
        } else if (language === 'uz') {
          if (answer.text.includes('jamoa')) insights.push('Jamoa ishini qadrlaydi');
          if (answer.text.includes('o\'rganish') || answer.text.includes('ta\'lim')) insights.push('O\'rganishga yuqori motivatsiya');
          if (answer.text.includes('o\'sish')) insights.push('O\'sishga yo\'naltirilgan');
        }
      }
    });
  ...
  private extractStrengths(answers: Answer[], language: Language): string[] {
    ...
    answers.forEach(answer => {
      if (answer.responseTime < 120 && answer.text.length > 80) {
        const strengthMessages: Record<Language, string> = {
          ja: '迅速で詳細な回答能力',
  ...
  private extractAreasForImprovement(answers: Answer[], language: Language): string[] {
    ...
    if (shortAnswers > answers.length * 0.3) {
      const improvementMessages: Record<Language, string> = {
        ja: 'より詳細な回答の提供',
